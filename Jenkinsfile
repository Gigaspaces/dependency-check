...
pipeline {
    ...
    stages {
        ...
        stage ('run') {
            steps {
                dependencyCheck(odcInstallation: 'dependency-check-v8.4.0', additionalArguments: "--project ${GS_FOLDER_NAME} --scan ${WORKSPACE}/build/${GS_FOLDER_NAME} --out ${WORKSPACE}/build/${GS_VERSION}/")
            }
        }
        /*
        stage('run') {
