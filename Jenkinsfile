...
pipeline {
    ...
    stages {
        ...
        stage ('run') {
            steps {
                dependencyCheck(odcInstallation: 'dependency-check-v8.4.0', additionalArguments: "--project ${GS_RELEASE_DIR} --scan ${WORKSPACE}/build/${GS_RELEASE_DIR} --out ${WORKSPACE}/build/${GS_VERSION}/")
            }
        }
        /*
        stage('run') {
